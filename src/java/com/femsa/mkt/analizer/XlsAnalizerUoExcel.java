...
public class XlsAnalizerUoExcel {
    ...
    private List<MktCobhDimUo> cargasInfoDatos = new ArrayList<MktCobhDimUo>();
    ...
    public List<MktCobhDimUo> getCargasInfoDatos() {
        return cargasInfoDatos;
    }

    public void setCargasInfoDatos(List<MktCobhDimUo> cargasInfoDatos) {
...
//                        System.out.println("ya entre ja"+ data.length);
    ...
    public boolean saveRows(long numRegistros, Long numProcesados, Long numFaltantes) throws XMLStreamException {
        ...
        boolean flagOk = true;
        MktCobhDimUoDao infoDatosDAO = new MktCobhDimUoDao();

        Session session = infoDatosDAO.getSession();
        List<MktCobhDimUo> cargas = new ArrayList<MktCobhDimUo>();
        MktCobhDimUo infoTemp = null;
        String elementName = "row";
        String[] data = null;
        rowNum = 0L;
        long cont = 0L;
            ...
            while (xmlReader.hasNext()) {
                ...
                if (xmlReader.isStartElement()) {
                    if (xmlReader.getLocalName().equals(elementName)) {
                        ...
                        infoTemp = new MktCobhDimUo();
                        try {
                            //Primera columna, string o nulo
                            infoTemp.setPkUo(data[0] != null && !data[0].equals("0") ? Long.parseLong(data[0]) : null);

                            //Segunda columna, string o nulo
                            infoTemp.setGvUo(data[1] != null ? data[1].trim().toUpperCase() : null);

                            //Tercera columna, string o nulo                            
//                            infoTemp.setFkEstado(data[2] != null && !data[2].equals("0") ? Long.parseLong(data[2]) : null);
                            
                            //Cuarto columna, string o nulo                            
//                            infoTemp.setFkRegion(data[3] != null && !data[3].equals("0") ? Long.parseLong(data[3]) : null);                            

                            //Quito columna, string o nulo                            
                            infoTemp.setUo(data[4] != null ? data[4].trim().toUpperCase() : null);
...
//                                System.out.println("entro a guardar en " + rowNum);
            ...
            session.createSQLQuery("INSERT INTO MKT_COBH_LOG (NOMBRE,TOTALREGISTROS,INICIO,FINAL,TIEMPO) VALUES('CARGA UO'," + numRegistros + ",TO_DATE('" + formatofecha.format(fechaInicio) + "', 'dd/mm/yyyy hh24:mi:ss'),TO_DATE('" + formatofecha.format(fechaFinal) + "', 'dd/mm/yyyy hh24:mi:ss')," + tiempo + ")").executeUpdate();
            ...
            if (ScriptAnalizer.executeScrits(errorsScript, "MKT_UO.sql")) {
                flagOk = true;
            } else {
                flagOk = false;
            }
