...
public class XlsAnalizerGecExcel {
    ...
    private List<MktCobhDimGec> cargasInfoDatos = new ArrayList<MktCobhDimGec>();
    ...
    public List<MktCobhDimGec> getCargasInfoDatos() {
        return cargasInfoDatos;
    }

    public void setCargasInfoDatos(List<MktCobhDimGec> cargasInfoDatos) {
...
//                        System.out.println("ya entre ja"+ data.length);
    ...
    public boolean saveRows(long numRegistros, Long numProcesados, Long numFaltantes) throws XMLStreamException {
        ...
        boolean flagOk = true;
        MktCobhDimGecDao infoDatosDAO = new MktCobhDimGecDao();

        Session session = infoDatosDAO.getSession();
        List<MktCobhDimGec> cargas = new ArrayList<MktCobhDimGec>();
        MktCobhDimGec infoTemp = null;
        String elementName = "row";
        String[] data = null;
        rowNum = 0L;
        long cont = 0L;
            ...
            while (xmlReader.hasNext()) {
                ...
                if (xmlReader.isStartElement()) {
                    if (xmlReader.getLocalName().equals(elementName)) {
                        ...
                        infoTemp = new MktCobhDimGec();
                        try {
                            //Primera columna, string o nulo
                            infoTemp.setPkGec(data[0] != null && !data[0].equals("0") ? Long.parseLong(data[0]) : null);

                            //Segunda columna, string o nulo
                            infoTemp.setGvGec(data[1] != null ? data[1].trim().toUpperCase() : null);

                            //tercera columna, string            
//                            infoTemp.setGec(data[2] != null ? data[2].trim().toUpperCase() : null);
...
//                                System.out.println("entro a guardar en " + rowNum);
                                for (MktCobhDimGec carga : cargas) {
            ...
            session.createSQLQuery("INSERT INTO MKT_COBH_LOG (NOMBRE,TOTALREGISTROS,INICIO,FINAL,TIEMPO) VALUES('CARGA GEC'," + numRegistros + ",TO_DATE('" + formatofecha.format(fechaInicio) + "', 'dd/mm/yyyy hh24:mi:ss'),TO_DATE('" + formatofecha.format(fechaFinal) + "', 'dd/mm/yyyy hh24:mi:ss')," + tiempo + ")").executeUpdate();
            ...
            if (ScriptAnalizer.executeScrits(errorsScript, "MKT_GEC.sql")) {
                flagOk = true;
            } else {
                flagOk = false;
            }
