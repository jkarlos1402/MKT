...
public class XlsAnalizerRegionExcel {
    ...
    private List<MktCobhDimRegion> cargasInfoDatos = new ArrayList<MktCobhDimRegion>();
    ...
    public List<MktCobhDimRegion> getCargasInfoDatos() {
        return cargasInfoDatos;
    }

    public void setCargasInfoDatos(List<MktCobhDimRegion> cargasInfoDatos) {
...
//                        System.out.println("ya entre ja"+ data.length);
    ...
    public boolean saveRows(long numRegistros, Long numProcesados, Long numFaltantes) throws XMLStreamException {
        ...
        boolean flagOk = true;
        MktCobhDimRegionDao infoDatosDAO = new MktCobhDimRegionDao();

        Session session = infoDatosDAO.getSession();
        List<MktCobhDimRegion> cargas = new ArrayList<MktCobhDimRegion>();
        MktCobhDimRegion infoTemp = null;
        String elementName = "row";
        String[] data = null;
        rowNum = 0L;
        long cont = 0L;
            ...
            while (xmlReader.hasNext()) {
                ...
                if (xmlReader.isStartElement()) {
                    if (xmlReader.getLocalName().equals(elementName)) {
                        ...
                        infoTemp = new MktCobhDimRegion();
                        try {
                            //Primera columna, string o nulo
                            infoTemp.setPkRegion(data[0] != null && !data[0].equals("0") ? Integer.parseInt(data[0]) : null);

                            //Segunda columna, string o nulo
                            infoTemp.setGvRegion(data[1] != null ? data[1].trim().toUpperCase() : null);
                            ...
                            infoTemp.setRegion(data[3] != null ? data[3].trim().toUpperCase() : null);
...
//                                System.out.println("entro a guardar en " + rowNum);
                                for (MktCobhDimRegion carga : cargas) {
            ...
            session.createSQLQuery("INSERT INTO MKT_COBH_LOG (NOMBRE,TOTALREGISTROS,INICIO,FINAL,TIEMPO) VALUES('CARGA REGION'," + numRegistros + ",TO_DATE('" + formatofecha.format(fechaInicio) + "', 'dd/mm/yyyy hh24:mi:ss'),TO_DATE('" + formatofecha.format(fechaFinal) + "', 'dd/mm/yyyy hh24:mi:ss')," + tiempo + ")").executeUpdate();
            ...
            if (ScriptAnalizer.executeScrits(errorsScript, "MKT_REGION.sql")) {
                flagOk = true;
            } else {
                flagOk = false;
            }
