...
public class XlsAnalizerCanalExcel {
    ...
    private List<MktCobhDimCanal> cargasInfoDatos = new ArrayList<MktCobhDimCanal>();
    ...
    public List<MktCobhDimCanal> getCargasInfoDatos() {
        return cargasInfoDatos;
    }

    public void setCargasInfoDatos(List<MktCobhDimCanal> cargasInfoDatos) {
...
//                        System.out.println("ya entre ja"+ data.length);
    ...
    public boolean saveRows(long numRegistros, Long numProcesados, Long numFaltantes) throws XMLStreamException {
        ...
        boolean flagOk = true;
        MktCobhDimCanalDao infoDatosDAO = new MktCobhDimCanalDao();

        Session session = infoDatosDAO.getSession();
        List<MktCobhDimCanal> cargas = new ArrayList<MktCobhDimCanal>();
        MktCobhDimCanal infoTemp = null;
        String elementName = "row";
        String[] data = null;
        rowNum = 0L;
        long cont = 0L;
            ...
            while (xmlReader.hasNext()) {
                ...
                if (xmlReader.isStartElement()) {
                    if (xmlReader.getLocalName().equals(elementName)) {
                        ...
                        infoTemp = new MktCobhDimCanal();
                        try {
                            //Primera columna, string o nulo
                            infoTemp.setPkCanal(data[0] != null && !data[0].equals("0") ? Long.parseLong(data[0]) : null);

                            //Segunda columna, string o nulo
                            infoTemp.setGvCanal(data[1] != null ? data[1].trim().toUpperCase() : null);

                            //tercera columna, string            
                            infoTemp.setCanaltexto(data[2] != null ? data[2].trim().toUpperCase() : null);
                            ...
                            if (rowNum % 10000 == 0 || rowNum == (numRegistros - 1)) {
                                ...
                                for (MktCobhDimCanal carga : cargas) {
...
//                            ex.printStackTrace();
            ...
            session.createSQLQuery("INSERT INTO MKT_COBH_LOG (NOMBRE,TOTALREGISTROS,INICIO,FINAL,TIEMPO) VALUES('CARGA CANAL'," + numRegistros + ",TO_DATE('" + formatofecha.format(fechaInicio) + "', 'dd/mm/yyyy hh24:mi:ss'),TO_DATE('" + formatofecha.format(fechaFinal) + "', 'dd/mm/yyyy hh24:mi:ss')," + tiempo + ")").executeUpdate();
            ...
            if (ScriptAnalizer.executeScrits(errorsScript, "MKT_CANAL.sql")) {
                flagOk = true;
            } else {
                flagOk = false;
            }
